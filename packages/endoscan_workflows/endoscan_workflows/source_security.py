...
class ScientificSourceClient:
    ...
    def _content_type_allowed(
        ...
        *,
        ...
        if not allow_official_geo_text:
            return False
        requested = urlparse(requested_url)
        final = urlparse(final_url)
        if (
        ...
        if params is None:
            requested_pairs = parse_qsl(requested.query, keep_blank_values=True)
        else:
            if requested.query:
                return False
            requested_pairs = [(str(key), str(value)) for key, value in params.items()]
        final_pairs = parse_qsl(final.query, keep_blank_values=True)
        required_keys = {"acc", "targ", "view", "form"}
        if (
            len(requested_pairs) != len(required_keys)
            or len(final_pairs) != len(required_keys)
            or {key for key, _value in requested_pairs} != required_keys
            or {key for key, _value in final_pairs} != required_keys
        ):
            return False
        requested_values = dict(requested_pairs)
        final_values = dict(final_pairs)
        if requested_values != final_values:
            return False
        accession = requested_values["acc"].upper()
        return (
            bool(GSE_ACCESSION.fullmatch(accession))
            and requested_values["targ"] == "self"
            and requested_values["form"] == "text"
            and requested_values["view"] in {"brief", "full"}
        )
