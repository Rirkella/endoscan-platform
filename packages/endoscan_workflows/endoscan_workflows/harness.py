    ...
    def _next_orchestrated_discovery_tool(
        ...
        if request.agent_name == "Transcriptomic Evidence Discovery Agent":
            ...
            for compound_index, compound in enumerate(compounds, start=1):
                ...
                for source_index, (operation, source_system) in enumerate(source_searches, start=1):
                    if operation not in request.available_tools:
                        continue
                    if any(
                        call_matches(
            ...
            for search_item in completed:
                ...
                if matched_cid is None:
                    continue
                compound = next(
                    (item for item in compounds if str(item["pubchem_cid"]).upper() == matched_cid),
                    None,
                )
                if compound is None:
                    continue
                ...
                identity_terms = [
                    matched_cid,
                    *([str(compound.get("inchikey"))] if compound.get("inchikey") else []),
                ]
                hydration_arguments = {
                    "source_system": (
                        "ncbi-geo-series"
