...
def test_specification_outcome_migration_upgrades_legacy_database_in_place(tmp_path) -> None:
    ...
    with database.engine.begin() as connection:
        ...
        connection.execute(
            text(
                "INSERT INTO training_dataset_workflows (workflow_id, specification_json) "
                "VALUES ('historical-build', NULL)"
            )
        )
        connection.execute(
            text(
    ...
    with database.engine.connect() as connection:
        semantics_version = connection.execute(
            text(
                "SELECT workflow_semantics_version FROM training_dataset_workflows "
                "WHERE workflow_id = 'historical-build'"
            )
        ).scalar_one()
...
def test_training_dataset_specification_approval_precedes_discovery(
    ...
    assert waiting.current_stage is WorkflowState.APPROVED_SPECIFICATION
    workflow = service.training_dataset_workflow(build.id)
    ...
    planned = service.continue_training_dataset_workflow(
        ...
        expected_version=waiting.version,
        actor="test-admin",
        idempotency_key="plan-source-discovery",
    )
...
def test_reviewed_source_authorization_stops_on_validated_provider_gaps(
    workflow_runtime,
) -> None:
    _database, _store, _providers, _harness, service = workflow_runtime
    ...
    planned = service.continue_training_dataset_workflow(
        ...
        actor="test-admin",
        idempotency_key="reviewed-source-authorization:plan",
    )
    readiness = service.source_discovery_readiness(build.id)
    assert readiness["ready"] is True
    assert readiness["workflow_semantics_version"] == "2.0.0"
    assert readiness["provider_execution_deferred"] is True
    assert readiness["discovery_plan_present"] is True
    assert readiness["provider_capability_blockers"] == []
    assert readiness["reviewed_adapters"]["ready"] is True
    workflow = service.training_dataset_workflow(build.id)
    assert workflow["workflow_semantics_version"] == "2.0.0"
    assert workflow["discovery_plan"] is not None
    assert workflow["provider_capability_findings"] is not None

    with pytest.raises(StaleWorkflowVersion):
        service.authorize_source_discovery(
            build.id,
            expected_version=planned.version - 1,
            actor="test-admin",
    ...
    with pytest.raises(InvalidTransition, match="DISCOVERY_PLANNING"):
        service.authorize_source_discovery(
            build.id,
...
def test_semantics_v2_authorization_never_runs_legacy_four_agent_orchestration(
    workflow_runtime,
) -> None:
    _database, _store, _providers, _harness, service = workflow_runtime
    build = service.create_build(
        EndpointBuildCreate(
            ...
            endpoint_slug="v2-no-legacy-agent-orchestration",
            biological_goal="Construct a compound-level transcriptomic training dataset.",
            created_by="test-admin",
            workflow_kind=WorkflowKind.TRAINING_DATASET_DISCOVERY,
            benchmark_mode="blind_training_dataset_discovery",
            idempotency_key="v2-no-legacy-agent-orchestration",
        )
    )
    waiting = service.start_build(
        build.id,
        expected_version=build.version,
        actor="test-admin",
        idempotency_key="v2-no-legacy-agent-orchestration:start",
    )
    approval = service.list_approvals(build.id, pending_only=True)[0]
    approved = service.decide_approval(
        approval["id"],
        ApprovalDecision(
            ...
            reviewer_id="test-admin",
            expected_version=waiting.version,
            idempotency_key="v2-no-legacy-agent-orchestration:approve",
            artifact_hashes=approval["request"]["artifact_hashes"],
            dataset_specification_policy=approved_specification_policy(),
        ),
    )
    service.derive_training_dataset_requirements(
        build.id,
        actor="deterministic-orchestrator",
        idempotency_key="v2-no-legacy-agent-orchestration:requirements",
    )
    planned = service.continue_training_dataset_workflow(
        build.id,
        expected_version=approved.version,
        actor="test-admin",
        idempotency_key="v2-no-legacy-agent-orchestration:plan",
    )
    authorized = service.authorize_source_discovery(
        build.id,
        expected_version=planned.version,
        actor="test-admin",
        idempotency_key="v2-no-legacy-agent-orchestration:authorize",
        confirmed=True,
    )
    assert authorized.current_stage is WorkflowState.DISCOVERING_SOURCE_CANDIDATES
    assert service.agent_runs(build.id) == []
    assert service.training_dataset_workflow(build.id)["source_observations"] == []


def _legacy_four_role_orchestration_reference(
    workflow_runtime,
) -> None:
    ...
    def official_fixture(request: httpx.Request) -> httpx.Response:
        ...
        if request.url.path.endswith("esearch.fcgi"):
            ...
            if database_name == "pcassay":
                identifier = (
                    "1001" if "binding" in query else "1003" if "antagonism" in query else "1002"
                )
            else:
                identifier = "2001"
            return httpx.Response(
...
def test_artifact_integrity_and_size_limit(workflow_runtime, tmp_path) -> None:
    _database, store, _providers, _harness, service = workflow_runtime
    build = create_build(service)
    ...
    derived = tmp_path / "reviewed-derived.sqlite"
    ...
    with pytest.raises(ArtifactTooLarge):
        store.put_file(
            ...
            source_path=derived,
            mime_type="application/octet-stream",
            artifact_type="test-derived",
            logical_name="unreviewed-derived.sqlite",
            producer="test",
            idempotency_key="unreviewed-derived",
        )
    reviewed = store.put_file(
        workflow_id=build.id,
        source_path=derived,
        mime_type="application/octet-stream",
        artifact_type="test-derived",
        logical_name="reviewed-derived.sqlite",
        producer="test",
        idempotency_key="reviewed-derived",
        reviewed_maximum_bytes=2 * 1024 * 1024,
    )
    assert reviewed.size_bytes == 1024 * 1024 + 1
