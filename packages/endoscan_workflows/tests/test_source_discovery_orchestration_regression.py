...
def _legacy_source_neutral_offline_four_agent_regression_paths(workflow_runtime, scenario) -> None:
    ...
    try:
        ...
        if scenario == "success":
            ...
            identity_rows = fragments["Chemical Identity and Structure Source Discovery Agent"][
            ...
            search_calls = [
                ...
                if call["tool_name"] in {"search_transcriptomic_sources", "search_lincs_resources"}
            ]
            assert search_calls
            assert all(
            ...
            assert all(
                "Synthetic compound" in call["arguments"]["normalized_arguments"]["query"]
                for call in search_calls
            )
