...
def test_reviewed_geo_study_type_aliases_are_canonicalized(supplied: str, canonical: str) -> None:
    normalized = normalize_geo_search_arguments(
    ...
    expected_warnings = (
        [] if supplied == canonical else ["controlled_vocabulary_alias_canonicalized"]
    )
...
def test_official_geo_text_mime_is_endpoint_scoped() -> None:
    ...
    for invalid_full_url in [
        f"{full_url}&extra=1",
        f"{full_url}&acc=GSE99999",
        full_url.replace("www.ncbi.nlm.nih.gov", "eutils.ncbi.nlm.nih.gov"),
        full_url.replace("/geo/query/acc.cgi", "/geo/query/other.cgi"),
    ]:
