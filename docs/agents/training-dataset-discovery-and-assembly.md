...
The `epa-comptox-toxcast@2.2.0` provider defaults to the unauthenticated `public_release` mode.
Clowder is the source of record for the v4.3 release hierarchy, exact file descriptors, archive
inventory, citation, checksums, and separate lightweight public workbooks. The provider normalizes
the complete assay annotation and target-mapping workbooks plus the public analytical-QC and
cytotoxicity chemical-reference tables. Release and dataset listings remain release metadata and
are never scientific candidates. The 7.50 GB summary archive and 17.65 GB SQL dump are never
downloaded automatically. A separately approved one-time staging operation verifies the summary
archive and required activity members; normal discovery then consumes only the immutable normalized
provider cache. A deployment without that staged cache reports the exact prerequisite instead of
requesting an API key or silently substituting PubChem.

The optional `authenticated_api` mode uses the official CTX Bioactivity API under
...
safe diagnostics. This mode is an acceleration surface, not the reproducibility contract. When the
key is absent, readiness reports `authenticated_api_unavailable`, keeps ToxCast in the DiscoveryPlan,
executes every `public_release` task, persists its completion evidence and capability findings, and
continues the remaining providers. The unauthenticated `/bioactivity/health` operation remains
