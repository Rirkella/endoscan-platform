...
describe("Analyze stepped flow + result overview", () => {
  ...
  it("below-threshold is NEUTRAL, not green/safe (fixture: both endpoints below threshold)", async () => {
    renderApp("/analyze");
    await analyze();
    // Both fixture endpoints are below threshold → neutral "Below threshold" chips, "Inactive" call.
    expect(screen.getAllByText(/Below threshold/i).length).toBe(2);
  ...
  it("renders one card per API result — scales beyond ER/AR without a fixed layout", async () => {
    ...
    renderApp("/analyze");
    await analyze();
    expect(await screen.findAllByRole("article")).toHaveLength(6); // one per API result
  });
});
